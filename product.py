    ...
    def add(self):
        ...
        try:
            ...
            if not price.isdigit() or int(price) <= 0:
             messagebox.showerror("Error","Price must be in digits",parent=self.root)
            else:
             
             qty = self.var_qty.get()
             if not qty.isdigit() or int(qty) <= 0:
              messagebox.showerror("Error","Quantity must be in digits",parent=self.root)
             else:
              if self.var_cat.get()=="Select"  or self.var_cat.get()=="Empty" or self.var_sup.get()=="Select" or self.var_name.get()=="":
                ...
                if row!=None:
                    ...
                    con.commit()
                    messagebox.showinfo("Successs","Product Added Successfully")
                    self.show()
        except Exception as ex:
            messagebox.showerror("Error",f"Error due to : {str(ex)}",parent=self.root)    
        ...
        try:
            if self.var_pid.get()=="":
                ...
                if row==None:
                    ...
                    con.commit()
                    messagebox.showinfo("Successs","Product Updated Successfully")
                    self.show()
        except Exception as ex:
            messagebox.showerror("Error",f"Error due to : {str(ex)}",parent=self.root) 
