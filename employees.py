...
class employeeClass:
    def __init__(self, root):
        ...
        self.root.title("Logistics Management System | Developed by Raj Gaund")
        ...
        # ====title====
        title = Label(self.root, text="Employees Details", font=("goudy old style", 15), bg="#0f4d7d", fg="white").place(x=50, y=100, width=1000)
        ...
        cmb_usertype=ttk.Combobox(self.root,textvariable=self.var_usertype,values=("Admin","Employee"),state='readonly',justify=CENTER,font=("goudy old style",15))
 ...
 #======================================================================================================================       
    ...
    def add(self):
        ...
        try:
           ...
           if not re.match(r'^\(?\d{3}\)?[\s-]?\d{3}[\s-]?\d{4}$', self.var_contact.get()):
             messagebox.showerror("Error", "Invalid contact format. Please use (XXX) XXX-XXXX", parent=self.root)
             return
           else:
           ...
           try:
            ...
            messagebox.showerror("Error", "Invalid DOB format. Use DD-MM-YYYY.", parent=self.root)
            return

           try:
             doj = datetime.strptime(self.var_doj.get(), "%d-%m-%Y")
           except ValueError:
            messagebox.showerror("Error", "Invalid DOJ format. Use DD-MM-YYYY.", parent=self.root)
            return

           password = self.var_password.get()
           if not re.match(r'^(?=.*[A-Z])(?=.*[a-z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$', password):
            messagebox.showerror("Error","Invalid Password Format",parent=self.root)
            return

           salary = self.var_salary.get()
           if not salary.isdigit() or int(salary) <= 0:
            messagebox.showerror("Error","Salary must be in digits",parent=self.root)
            return

           address = self.txt_address.get('1.0', 'end').strip() 
           if not address or len(address) < 6:
            messagebox.showerror("Error","Addres must be Characters, at least 6 characters",parent=self.root)
            return
           ...
           if row!=None:
                    ...
                    con.commit()
                    messagebox.showinfo("Successs","Employee Added Successfully")
                    self.show()
        ...
        try:
            if self.var_emp_id.get()=="":
                ...
                if row==None:
                    ...
                    con.commit()
                    messagebox.showinfo("Successs","Employee Updated Successfully")
                    self.show()
        except Exception as ex:
            messagebox.showerror("Error",f"Error due to : {str(ex)}",parent=self.root) 
